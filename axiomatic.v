...
Inductive ax_stmt_post' `{Env K} (Pd : dassert K) (s : stmt K)
    (Γ : env K) (Δ : memenv K) (ρ : stack K) : focus K → mem K → Prop :=
  mk_ax_stmt_post d m :
    direction_out d s →
    mem_locks m = ∅ →
    assert_holds (Pd d) Γ Δ ρ (cmap_erase m) →
    ax_stmt_post' Pd s Γ Δ ρ (Stmt d s) m.
Program Definition ax_stmt_post `{Env K}
    (Pd : dassert K) (s : stmt K) : ax_assert K := {|
  ax_assert_holds := ax_stmt_post' Pd s
|}.
Next Obligation.
  intros ?? Pd s Γ1 Γ2 Δ1 Δ2 ρ φ m ???; destruct 1 as [d m' ???];
    constructor; eauto using assert_weaken, cmap_erase_valid.
Qed.
Next Obligation.
  intros ?? Pd s Γ δ Δ ρ φ m m' [d m'' ???] [? p]; inv_rcstep.
Qed.
...
Inductive ax_expr_cond_frame `{Env K}
     (ρ : stack K) (A : assert K) (Γ : env K) (Δ : memenv K)
     (k : ctx K) (φ : focus K) (m m' : mem K) : ax_expr_frame K → Prop :=
  | ax_frame_in_expr mA mf :
     m' = m ∪ mf ∪ mA → ✓{Γ,Δ} mf → ✓{Γ,Δ} mA →
     ⊥ [m; mf; mA] → k = [] → cmap_erased mA → mem_locks mA = ∅ →
     assert_holds A Γ Δ ρ mA →
     ax_expr_cond_frame ρ A Γ Δ k φ m m' (InExpr mf mA)
  | ax_frame_in_fun mf :
     m' = m ∪ mf → ✓{Γ,Δ} mf →
     ⊥ [m; mf] → k ≠ [] →
     ax_expr_cond_frame ρ A Γ Δ k φ m m' (InFun mf).
Inductive ax_expr_cond_unframe `{Env K}
     (ρ : stack K) (A : assert K) (Γ : env K) (Δ : memenv K)
     (k : ctx K) (φ : focus K) (m m' : mem K) : ax_expr_frame K → Prop :=
  | ax_unframe_expr_to_expr mA mf :
     m' = m ∪ mf ∪ mA → ⊥ [m; mf; mA] → k = [] →
     ax_expr_cond_unframe ρ A Γ Δ k φ m m' (InExpr mf mA)
  | ax_unframe_fun_to_expr mA mf :
     m' = m ∪ mf ∪ mA → ⊥ [m; mf; mA] → k = [] →
     cmap_erased mA → mem_locks mA = ∅ →
     assert_holds A Γ Δ ρ mA → 
     ax_expr_cond_unframe ρ A Γ Δ k φ m m' (InFun mf)
...
Definition ax_expr `{EnvSpec K} (Γ : env K) (δ : funenv K) (A P : assert K)
  ...
  locks e = ∅ →
  ✓{Δ}* ρ →
  ax_expr_funframe Γ Δ A ρ m →
  assert_holds P Γ Δ ρ (cmap_erase m) →
...
Proof.
  ...
  eapply ax_weaken; [| |eapply Hax| | |]; eauto; [eapply HPQ; eauto|].
  destruct 3; constructor; auto. eapply HPQ; eauto using indexes_valid_weaken.
Qed.
...
Lemma ax_expr_funframe_emp Γ Δ ρ m : ✓{Γ,Δ} m → ax_expr_funframe Γ Δ emp ρ m.
Proof. by eexists ∅; split_ands; eauto. Qed.
Hint Resolve ax_expr_funframe_emp.
Lemma ax_expr_funframe_weaken Γ Δ A ρ m1 m2 :
  ax_expr_funframe Γ Δ A ρ m2 → m1 ⊆ m2 → ax_expr_funframe Γ Δ A ρ m1.
Proof.
  intros (mA&?&?&?) Hm12; exists mA; split; auto.
...
Proof.
  intros Δ k φ a m m' ?; simpl. destruct 1; subst; [discriminate_list_equality|].
...
Proof.
  intros Δ k' φ m m' mf ? (->&?&?); simpl; destruct (decide (k' = [])) as [->|].
...
Proof.
  intros Δ k φ m m' ?; destruct 4 as [mA mf'|mf']; subst.
...
Proof.
  ...
  eapply ax_weaken; [| |apply Hax| | |]; eauto.
  * econstructor; split_ands; eauto. apply HA; eauto.
  * apply HP; eauto.
  * destruct 3; constructor; auto. apply HA; eauto using indexes_valid_weaken.
  * destruct 3; subst; simplify_mem_disjoint_hyps;
...
Lemma ax_expr_post_expr_nf Γ Q Δ ρ m e τlr :
  ax_expr_post' Q τlr Γ Δ ρ (Expr e) m → is_nf e.
Proof. by inversion 1. Qed.
Lemma ax_expr_lrval Γ δ A (P : lrval K → assert K) Δ ρ n ν Ω m τlr :
  ✓{Γ,Δ} m → ax_expr_funframe Γ Δ A ρ m →
  ax_graph (ax_expr_cond ρ A)
    (ax_expr_post P τlr) Γ δ Δ ρ (S n) [] (Expr (%#{Ω} ν)%E) m →
  assert_holds (P ν) Γ Δ ρ (cmap_erase m) ∧ mem_locks m = Ω ∧ (Γ,Δ) ⊢ ν : τlr.
Proof.
  intros ? (mA&?&?&?&?&?);
    inversion 1 as [|??? [??????]|???? Hred]; subst; auto.
  destruct (Hred Δ (m ∪ mA) (InExpr ∅ mA)); auto; [|inv_rcstep].
  constructor; eauto. by rewrite sep_right_id by auto.
Qed.
...
Lemma ax_call_compose Γ δ A P Q (E : ectx K) E' ρ Δ n φ k m1 m2 :
  ...
  (∀ Δ' n' m1' e',
    n' ≤ n →
    ⊥ [m1'; m2] → ✓{Γ,Δ'} m1' → ✓{Γ,Δ'} m2 → Δ ⇒ₘ Δ' →
    ax_expr_funframe Γ Δ' A ρ (m1' ∪ m2) →
...
Proof.
  ...
  induction n as [|n IH]; intros Δ k φ m1 ??? Hax Hnext; [apply ax_O |].
  inversion Hax as [| |???? Hred Hstep];
    clear Hax; subst; try discriminate_list_equality.
  apply ax_further.
  { intros Δ' m' a ?? Hframe; inversion_clear Hframe;
      subst; try discriminate_list_equality; simplify_mem_disjoint_hyps.
    destruct (Hred Δ' (m1 ∪ m2 ∪ mf) (InFun (m2 ∪ mf))) as [S' p]; auto.
  ...
  * intros k' φ' m' p' ?.
    ...
    inversion_clear Hunframe as [|mA| |]; subst;
      try discriminate_list_equality; simplify_mem_disjoint_hyps.
    apply mk_ax_next with Δ'' (m1' ∪ m2); eauto 6 using focus_locks_valid_union.
    rewrite sep_associative by auto. constructor; auto.
    intro; discriminate_list_equality.
  * intros f v -> -> p' ?.
    feed inversion (Hstep Δ' (m1 ∪ m2 ∪ mf) (InFun (m2 ∪ mf))
...
Qed.
Lemma ax_expr_compose Γ δ n {vn} A (Ps : vec (vassert K) vn) (Q : vassert K)
    Δ (E : ectx_full K vn) (es : vec (expr K) vn) (ρ : stack K)
    (ms : vec (mem K) vn) (τlrs : vec (lrtype K) vn) σlr :
  ✓ Γ → locks E = ∅ → ✓{Δ}* ρ →
  ⊥ ms → (∀ i, ✓{Γ,Δ} (ms !!! i)) →
  ax_expr_funframe Γ Δ A ρ (⋃ ms) →
  (∀ i, locks (es !!! i) ⊆ mem_locks (ms !!! i)) →
  (∀ i, ax_graph (ax_expr_cond ρ A) (ax_expr_post (Ps !!! i) (τlrs !!! i))
      Γ δ Δ ρ n [] (Expr (es !!! i)) (ms !!! i)) →
  (∀ Δ' (Ωs : vec _ vn) (νs : vec _ vn) (ms' : vec _ vn),
    ⊥ ms' → (∀ i, ✓{Γ,Δ'} (ms' !!! i)) → Δ ⇒ₘ Δ' →
    (∀ i, (Γ,Δ') ⊢ νs !!! i : τlrs !!! i) →
    (∀ i, mem_locks (ms' !!! i) = Ωs !!! i) →
    (∀ i,
      assert_holds ((Ps !!! i) (νs !!! i)) Γ Δ' ρ (cmap_erase (ms' !!! i))) →
...
Proof.
  ...
  apply ax_further.
  { intros Δ' ???? Hframe; inversion_clear Hframe;
      simplify_equality'; simplify_mem_disjoint_hyps.
  ...
  intros Δ' m' a S' ?? Hframe p; inversion_clear Hframe as [mA mf ??? Hm|];
    simplify_equality'; simplify_mem_disjoint_hyps.
  ...
  * clear i Hi; intros i e' m2 p' ?.
    ...
    feed inversion (ax_step (ax_expr_cond ρ A)
      ...
      (State [] (Expr e') m2)) as [? Δ'' ?? m ?? Hunframe _]; subst; auto.
    { eauto using cmap_union_list_lookup_valid. }
    ...
    + intros j; destruct (decide (i = j)); subst; [by rewrite !vlookup_insert|].
      rewrite !vlookup_insert_ne by done.
      apply ax_S, ax_graph_weaken with Δ; eauto.
    + intros Δ''' Ωs vs ms' ??????. apply ax_S, Hax2; auto.
  * clear i Hi. intros i E' Ω f τs τ Ωs vs ? Hesi p ?.
    ...
    { transitivity (locks (es !!! i)); [|done].
      ...
      rewrite fmap_zip_with_l by auto with lia; clear; solve_elem_of. }
    ...
    feed inversion (ax_step (ax_expr_cond ρ A)
      ...
      as [? Δ'' ?? m ?? Hunframe _ ? Hm']; trivial; subst.
    { eauto using cmap_union_list_lookup_valid. }
    { constructor; auto.
      { by rewrite sep_associative, sep_union_delete by auto. }
      apply cmap_union_valid_2; auto.
    ...
    + intros j; destruct (decide (i = j)); subst; [by rewrite !vlookup_insert|].
      rewrite !vlookup_insert_ne by done.
      apply ax_graph_weaken with Δ; eauto using ax_le.
    + intros Δ'''' Ωs' νs' ms' ??????.
  ...
  * clear i Hi. intros i E' e ? p ?.
    ...
    apply cmap_union_list_valid, Forall_delete, Forall_vlookup;
      eauto using cmap_union_list_lookup_valid.
Qed.
Lemma ax_expr_compose_1 Γ δ n A P Q Δ (E : ectx_full K 1) e ρ m τlr σlr :
  ✓ Γ → locks E = ∅ → ✓{Δ}* ρ → ✓{Γ,Δ} m →
  ax_expr_funframe Γ Δ A ρ m →
  locks e ⊆ mem_locks m →
  ax_graph (ax_expr_cond ρ A) (ax_expr_post P τlr) Γ δ Δ ρ n [] (Expr e) m →
  (∀ Δ' Ω ν m',
    ✓{Γ,Δ'} m' → Δ ⇒ₘ Δ' → (Γ,Δ') ⊢ ν : τlr → mem_locks m' = Ω →
    assert_holds (P ν) Γ Δ' ρ (cmap_erase m') →
    ax_graph (ax_expr_cond ρ A) (ax_expr_post Q σlr) Γ δ Δ' ρ n []
...
Proof.
  ...
  apply (ax_expr_compose Γ δ n A [# P]%A _ Δ E [# e] ρ [# m] [# τlr]); simpl;
    rewrite ?sep_right_id by auto; auto; try (by intros; inv_all_vec_fin).
  intros Δ' Ωs νs ms' ? Hm' ? Hν Hlocks Hax'; inv_all_vec_fin; simpl.
  apply Hax; simpl; rewrite ?sep_right_id by auto; auto;
    apply (Hm' 0%fin) || apply (Hν 0%fin) ||
    apply (Hlocks 0%fin) || apply (Hax' 0%fin).
Qed.
Lemma ax_expr_compose_2 Γ δ n A P1 P2 Q Δ
    (E : ectx_full K 2) e1 e2 ρ m1 m2 τlr1 τlr2 σlr :
  ...
  (∀ Δ' Ω1 Ω2 ν1 ν2 m1' m2',
    ...
    assert_holds (P1 ν1) Γ Δ' ρ (cmap_erase m1') →
    assert_holds (P2 ν2) Γ Δ' ρ (cmap_erase m2') →
    ax_graph (ax_expr_cond ρ A) (ax_expr_post Q σlr) Γ δ Δ' ρ n []
...
Proof.
  ...
  apply (ax_expr_compose Γ δ n A [# P1; P2]%A Q Δ E
    [# e1; e2] ρ [# m1; m2] [# τlr1; τlr2] σlr); simpl;
    rewrite ?sep_right_id by auto; auto; try (by intros; inv_all_vec_fin).
  intros Δ' Ωs νs ms' ? Hm' ? Hν Hlocks Hax'; inv_all_vec_fin; simpl.
  apply Hax; simpl; rewrite ?sep_right_id by auto; auto;
    repeat match goal with
