...
Lemma expr_eval_ehstep Γ fs ρ e1 m ν :
  ⟦ e1 ⟧ Γ fs ρ m = Some ν → is_redex e1 →
  (**i 1). *) (∃ e2, Γ\ ρ ⊢ₕ e1, m ⇒ e2, m ∧ ⟦ e2 ⟧ Γ fs ρ m = Some ν) ∨
  (**i 2). *) (∃ Ω f τs τ F Ωs vs v,
    e1 = (call #{Ω} (ptrV (FunPtr f τs τ)) @ #{Ωs}* vs)%E ∧ ν = inr v ∧
...
Qed.
Lemma expr_eval_subst_ehstep Γ fs ρ m E e1 ν :
  ⟦ subst E e1 ⟧ Γ fs ρ m = Some ν → is_redex e1 →
  (**i 1). *) (∃ e2,
    Γ\ ρ ⊢ₕ e1, m ⇒ e2, m ∧ ⟦ subst E e2 ⟧ Γ fs ρ m = Some ν) ∨
  (**i 2). *) (∃ Ω f τs τ F Ωs vs v,
    e1 = (call #{Ω} (ptrV (FunPtr f τs τ)) @ #{Ωs}* vs)%E ∧
    length Ωs = length vs ∧ fs !! f = Some F ∧ F vs = Some v ∧
    ⟦ subst E (# v)%E ⟧ Γ fs ρ m = Some ν).
Proof.
  rewrite expr_eval_subst; intros (ν'&Heval'&<-) ?.
  destruct (expr_eval_ehstep _ _ _ _ _ _ Heval')
    as [(e2&?&?)|(Ω&f&τs&τ&F&Ωs&vs&v&->&->&?&?&?)]; trivial.
  * left; exists e2; split; auto using subst_preserves_expr_eval.
...
Qed.
Lemma expr_eval_subst_ehsafe Γ fs ρ m E e ν :
  ⟦ subst E e ⟧ Γ fs ρ m = Some ν → is_redex e → Γ\ ρ ⊢ₕ safe e, m.
Proof.
  intros Heval ?. destruct (expr_eval_subst_ehstep Γ fs ρ m E e ν)
    as [(e2&?&?)|(Ω&f&τs&τ&F&Ωs&vs&v&->&?&?&?&?)]; trivial.
