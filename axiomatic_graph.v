...
Record ax_frame_cond (K : Set) `{Env K} (A : Type) : Type := {
  frame :
    env K → memenv K → ctx K → nat → focus K → mem K → mem K → A → Prop;
  unframe :
    env K → memenv K → ctx K → nat → focus K → mem K → mem K → A → Prop;
  frame_valid Γ Δ k n φ m m' a :
    ✓ Γ → frame Γ Δ k n φ m m' a → ✓{Γ,Δ} m → ✓{Γ,Δ} m';
  unframe_valid Γ Δ k n φ m m' a :
    ✓ Γ → unframe Γ Δ k n φ m m' a → ✓{Γ,Δ} m' → ✓{Γ,Δ} m
}.
Arguments frame {_ _ _} _ _ _ _ _ _ _ _ _.
Arguments unframe {_ _ _} _ _ _ _ _ _ _ _ _.

Record ax_assert (K : Set) `{Env K} : Type := {
  ax_assert_holds :
    env K → memenv K → stack K → nat → focus K → mem K → Prop;
  ax_assert_weaken Γ1 Γ2 Δ1 Δ2 ρ n n' φ m :
    ✓ Γ1 → ✓{Γ1,Δ1} m → ✓{Δ1}* ρ →
    ax_assert_holds Γ1 Δ1 ρ n φ m →
    Γ1 ⊆ Γ2 → Δ1 ⇒ₘ Δ2 → n' ≤ n →
    ax_assert_holds Γ2 Δ2 ρ n' φ m;
  ax_assert_nf Γ δ Δ ρ n φ m m' :
    ax_assert_holds Γ Δ ρ n φ m → nf (rcstep Γ δ ρ) (State [] φ m')
}.
Arguments ax_assert_holds {_ _} _ _ _ _ _ _ _.
...
Section basics.
  ...
  Inductive ax_graph (Δ : memenv K) (ρ : stack K) :
    ...
    | ax_done n φ m :
       ax_assert_holds P Γ Δ ρ n φ m → ax_graph Δ ρ n [] φ m
    | ax_further n k φ m :
       (∀ Δ' n' m' a,
         ✓{Γ,Δ'} m → Δ ⇒ₘ Δ' → n' < n →
...
Proof.
  intros Δ k1 n φ m m' a; rewrite (right_id_L [] (++)); exists a; split; auto.
...
Lemma ax_compose {A B} (F : ax_frame_cond K A) (G : ax_frame_cond K B)
  ...
  (∀ Δ' n' φ' m',
    ax_assert_holds P Γ Δ' (rlocals ρ k2) n' φ' m' →
    ✓{Γ,Δ'} m' → Δ ⇒ₘ Δ' → n' ≤ n →
...
Lemma ax_compose_cons {A B} (F : ax_frame_cond K A) (G : ax_frame_cond K B)
  ...
  (∀ Δ' n' φ' m',
    ax_assert_holds P Γ Δ' (rlocals ρ [Ek]) n' φ' m' →
    ✓{Γ,Δ'} m' → Δ ⇒ₘ Δ' → n' ≤ n →
...
Lemma ax_frame {A B}(F : ax_frame_cond K A) (G : ax_frame_cond K B)
  ...
  (∀ Δ' n' φ' m',
    ...
    ax_assert_holds P Γ Δ' ρ n' φ' m' →
    ax_assert_holds Q Γ Δ' ρ n' φ' (m' ∪ mf)) →
  ax_graph G Q Γ δ Δ ρ n k φ (m ∪ mf).
Proof.
  intros ? HFG. revert Δ m k φ.
  induction n as [[|n] IH] using lt_wf_ind; [intros; apply ax_O|].
...
Lemma ax_weaken {A} (F G : ax_frame_cond K A)
  ...
  (∀ Δ' k' n' φ' m2 m2' a,
    ✓{Γ,Δ'} m2 →
    frame G Γ Δ' k' n' φ' m2 m2' a →
    Δ ⇒ₘ Δ' → n' ≤ n →
    frame F Γ Δ' k' n' φ' m2 m2' a) →
  (∀ Δ' k' n' φ' m2 m2' a,
    ✓{Γ,Δ'} m2' →
    unframe F Γ Δ' k' n' φ' m2 m2' a →
    Δ ⇒ₘ Δ' → n' ≤ n →
    unframe G Γ Δ' k' n' φ' m2 m2' a) →
  (∀ Δ' n' φ' m',
    ✓{Γ,Δ'} m' →
    ax_assert_holds P Γ Δ' ρ n' φ' m' →
    Δ ⇒ₘ Δ' → n' ≤ n →
    ax_assert_holds Q Γ Δ' ρ n' φ' m') →
...
Proof.
  ...
  inversion Hax as [| |???? Hred Hstep]; subst; eauto using ax_done.
  { apply ax_done. eapply ax_assert_weaken with _ _ (S n); eauto. }
  apply ax_further; eauto.
  { intros Δ' n'' m' a ????; eapply (Hred Δ' n'' m' a); eauto. }
  intros Δ' n'' m' a S' ???? p ?.
  feed inversion (Hstep Δ' n'' m' a S'); subst; eauto.
  econstructor; eauto.
  eapply (IH n''); eauto using unframe_valid, indexes_valid_weaken.
Qed.
...
Program Definition ax_disjoint_cond : ax_frame_cond K (mem K) := {|
  frame Γ Δ k n φ m m' mf := m' = m ∪ mf ∧ ✓{Γ,Δ} mf ∧ ⊥ [m; mf];
  unframe Γ Δ k n φ m m' mf := m' = m ∪ mf ∧ ⊥ [m; mf]
|}.
...
Proof.
  intros ? Δ k n φ m m' mf' ??? (->&?&?). exists (mf ∪ mf'); split.
...
Proof.
  intros Δ k n φ m ? mf ? (->&?&?); exists mf; split_ands; eauto. by split.
Qed.
End ax_graph.
