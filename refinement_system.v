...
Inductive lrval_refine' (Γ : env Ti) (α : bool)
  ...
  | lval_refine a1 a2 τ :
     a1 ⊑{Γ,α,f@Γm1↦Γm2} a2 : TType τ → addr_strict Γ a1 →
...
Inductive expr_refine' (Γ : env Ti)
  ...
  | EVar_refine τ n :
     τs !! n = Some τ →
     expr_refine' Γ τs α f Γm1 Γm2 (var{τ} n) (var{τ} n) (inl τ)
  | EVal_refine Ω1 Ω2 v1 v2 τ :
     Ω1 ⊑{Γ,α,f@Γm1↦Γm2} Ω2 → v1 ⊑{Γ,α,f@Γm1↦Γm2} v2 : τ →
     expr_refine' Γ τs α f Γm1 Γm2 (#{Ω1} v1) (#{Ω2} v2) (inr τ)
  | EAddr_refine Ω1 Ω2 a1 a2 τ :
     Ω1 ⊑{Γ,α,f@Γm1↦Γm2} Ω2 →
     a1 ⊑{Γ,α,f@Γm1↦Γm2} a2 : TType τ → addr_strict Γ a1 →
     expr_refine' Γ τs α f Γm1 Γm2 (%{Ω1} a1) (%{Ω2} a2) (inl τ)
  | ERtoL_refine e1 e2 τ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr (TType τ.*)) → type_complete Γ τ →
     expr_refine' Γ τs α f Γm1 Γm2 (.* e1) (.* e2) (inl τ)
  | ERofL_refine e1 e2 τ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inl τ) →
     expr_refine' Γ τs α f Γm1 Γm2 (& e1) (& e2) (inr (TType τ.*))
  | EAssign_refine ass e1 e2 e1' e2' τ τ' σ :
     assign_typed Γ τ τ' ass σ →
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inl τ) →
     expr_refine' Γ τs α f Γm1 Γm2 e1' e2' (inr τ') →
     expr_refine' Γ τs α f Γm1 Γm2
       (e1 ::={ass} e1') (e2 ::={ass} e2') (inr σ)
  | ECall_refine e1 e2 es1 es2 σ σs :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr ((σs ~> σ).*)) →
     Forall3 (expr_refine' Γ τs α f Γm1 Γm2) es1 es2 (inr <$> σs) →
     type_complete Γ σ →
     expr_refine' Γ τs α f Γm1 Γm2 (call e1 @ es1) (call e2 @ es2) (inr σ)
  | EAbort_refine τ :
     ✓{Γ} τ → expr_refine' Γ τs α f Γm1 Γm2 (abort τ) (abort τ) (inr τ)
  | ELoad_refine e1 e2 τ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inl τ) →
     expr_refine' Γ τs α f Γm1 Γm2 (load e1) (load e2) (inr τ)
  | EEltL_refine e1 e2 rs τ σ  :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inl τ) → Γ ⊢ rs : τ ↣ σ →
     expr_refine' Γ τs α f Γm1 Γm2 (e1 %> rs) (e2 %> rs) (inl σ)
  | EEltR_refine e1 e2 rs τ σ  :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr τ) → Γ ⊢ rs : τ ↣ σ →
     expr_refine' Γ τs α f Γm1 Γm2 (e1 #> rs) (e2 #> rs) (inr σ)
  | EAlloc_refine τ e1 e2 τi :
     ✓{Γ} τ → expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr (intT τi)) →
     expr_refine' Γ τs α f Γm1 Γm2 (alloc{τ} e1) (alloc{τ} e2) (inr (TType τ.*))
  | EFree_refine e1 e2 τ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inl τ) →
     expr_refine' Γ τs α f Γm1 Γm2 (free e1) (free e2) (inr voidT)
  | EUnOp_refine op e1 e2 τ σ :
     unop_typed op τ σ → expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr τ) →
     expr_refine' Γ τs α f Γm1 Γm2 (@{op} e1) (@{op} e2) (inr σ)
  | EBinOp_refine op e1 e2 e1' e2' τ τ' σ :
     binop_typed op τ τ' σ → expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr τ) →
     expr_refine' Γ τs α f Γm1 Γm2 e1' e2' (inr τ') →
     expr_refine' Γ τs α f Γm1 Γm2 (e1 @{op} e1') (e2 @{op} e2') (inr σ)
  | EIf_refine e1 e2 e1' e2' e1'' e2'' τb σ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr (baseT τb)) →
     expr_refine' Γ τs α f Γm1 Γm2 e1' e2' (inr σ) →
     expr_refine' Γ τs α f Γm1 Γm2 e1'' e2'' (inr σ) →
     expr_refine' Γ τs α f Γm1 Γm2
       (if{e1} e1' else e1'') (if{e2} e2' else e2'') (inr σ)
  | EComma_refine e1 e2 e1' e2' τ1 τ2 :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr τ1) →
     expr_refine' Γ τs α f Γm1 Γm2 e1' e2' (inr τ2) →
     expr_refine' Γ τs α f Γm1 Γm2 (e1 ,, e1') (e2 ,, e2') (inr τ2)
  | ECast_refine e1 e2 τ σ :
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr τ) → cast_typed Γ τ σ → 
     expr_refine' Γ τs α f Γm1 Γm2 (cast{σ} e1) (cast{σ} e2) (inr σ)
  | EInsert_refine r e1 e2 e1' e2' τ σ :
     Γ ⊢ r : τ ↣ σ →
     expr_refine' Γ τs α f Γm1 Γm2 e1 e2 (inr σ) →
     expr_refine' Γ τs α f Γm1 Γm2 e1' e2' (inr τ) →
     expr_refine' Γ τs α f Γm1 Γm2 (#[r:=e1]e1') (#[r:=e2]e2') (inr τ).
...
Section expr_refine_ind.
  ...
  Context (Palloc : ∀ τ e1 e2 τi,
    ✓{Γ} τ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr (intT τi) →
  ...
  Context (Punop : ∀ op e1 e2 τ σ,
    unop_typed op τ σ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr τ →
  ...
  Context (Pif : ∀ e1 e2 e1' e2' e1'' e2'' τb σ,
    e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr (baseT τb) → P e1 e2 (inr (baseT τb)) →
    e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr σ → P e1' e2' (inr σ) →
    e1'' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2'' : inr σ → P e1'' e2'' (inr σ) →
  ...
  Lemma expr_refine_ind : ∀ e1 e2 τ,
    expr_refine' Γ τs α f Γm1 Γm2 e1 e2 τ → P e1 e2 τ.
...
Inductive ectx_item_refine' (Γ : env Ti) (τs: list (type Ti))
  ...
  | CRtoL_refine τ :
     type_complete Γ τ →
     ectx_item_refine' Γ τs α f Γm1 Γm2 (.* □) (.* □) (inr (TType τ.*)) (inl τ)
  | CLtoR_refine τ :
     ectx_item_refine' Γ τs α f Γm1 Γm2 (& □) (& □) (inl τ) (inr (TType τ.*))
  | CAssignL_refine ass e1' e2' τ τ' σ :
     assign_typed Γ τ τ' ass σ → e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr τ' →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (□ ::={ass} e1') (□ ::={ass} e2') (inl τ) (inr σ)
  | CAssignR_refine ass e1 e2 τ τ' σ :
     assign_typed Γ τ τ' ass σ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inl τ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (e1 ::={ass} □) (e2 ::={ass} □) (inr τ') (inr σ)
  | CCallL_refine es1 es2 σs σ :
     es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* inr <$> σs → type_complete Γ σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (call □ @ es1) (call □ @ es2) (inr ((σs ~> σ).*)) (inr σ)
  | CCallR_refine e1 e2 es1 es2 es1' es2' σ σs τ σs' :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr (((σs ++ τ :: σs') ~> σ).*) →
     reverse es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* reverse es2 :* inr <$> σs →
     es1' ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2' :* inr <$> σs' → type_complete Γ σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (call e1 @ es1 □ es1') (call e2 @ es2 □ es2') (inr τ) (inr σ)
  | CLoad_refine τ :
     ectx_item_refine' Γ τs α f Γm1 Γm2 (load □) (load □) (inl τ) (inr τ)
  | CEltL_refine rs τ σ :
     Γ ⊢ rs : τ ↣ σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2 (□ %> rs) (□ %> rs) (inl τ) (inl σ)
  | CEltR_refine rs τ σ :
     Γ ⊢ rs : τ ↣ σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2 (□ #> rs) (□ #> rs) (inr τ) (inr σ)
  | CAlloc_refine τ τi :
     ✓{Γ} τ → ectx_item_refine' Γ τs α f Γm1 Γm2
       (alloc{τ} □) (alloc{τ} □) (inr (intT τi)) (inr (TType τ.*))
  | CFree_refine τ :
     ectx_item_refine' Γ τs α f Γm1 Γm2 (free □) (free □) (inl τ) (inr voidT)
  | CUnOp_refine op τ σ :
     unop_typed op τ σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2 (@{op} □) (@{op} □) (inr τ) (inr σ)
  | CBinOpL_refine op e1' e2' τ τ' σ :
     binop_typed op τ τ' σ → e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr τ' →
     ectx_item_refine' Γ τs α f Γm1 Γm2
  ...
  | CIf_refine e1' e2' e1'' e2'' τb σ :
     e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr σ →
     e1'' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2'' : inr σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (if{□} e1' else e1'') (if{□} e2' else e2'') (inr (baseT τb)) (inr σ)
  | CComma_refine e1' e2' τ τ' :
     e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr τ' →
     ectx_item_refine' Γ τs α f Γm1 Γm2 (□,, e1') (□,, e2') (inr τ) (inr τ')
  | CCast_refine τ σ :
     cast_typed Γ τ σ → ectx_item_refine' Γ τs α f Γm1 Γm2
       (cast{σ} □) (cast{σ} □) (inr τ) (inr σ)
  | CInsertL_refine r e1' e2' τ σ :
     Γ ⊢ r : τ ↣ σ → e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2' : inr τ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
       (#[r:=□]e1') (#[r:=□]e2') (inr σ) (inr τ)
  | CInsertR_refine r e1 e2 τ σ :
     Γ ⊢ r : τ ↣ σ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr σ →
     ectx_item_refine' Γ τs α f Γm1 Γm2
...
Inductive stmt_refine' (Γ : env Ti) (τs : list (type Ti))
  ...
  | SSkip_refine : stmt_refine' Γ τs α f Γm1 Γm2 skip skip (false,None)
  | SDo_refine e1 e2 τ :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr τ →
     stmt_refine' Γ τs α f Γm1 Γm2 (! e1) (! e2) (false,None)
  | SGoto_refine l :
     stmt_refine' Γ τs α f Γm1 Γm2 (goto l) (goto l) (true,None)
  | SThrow_refine n :
     stmt_refine' Γ τs α f Γm1 Γm2 (throw n) (throw n) (true,None)
  | SReturn_refine e1 e2 τ :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr τ →
     stmt_refine' Γ τs α f Γm1 Γm2 (ret e1) (ret e2) (true,Some τ)
  | SLabel_refine l :
     stmt_refine' Γ τs α f Γm1 Γm2 (label l) (label l) (false,None)
  | SLocal_refine' τ s1 s2 c mσ :
     ✓{Γ} τ → int_typed (size_of Γ τ) sptrT →
     stmt_refine' Γ (τ :: τs) α f Γm1 Γm2 s1 s2 (c,mσ) →
     stmt_refine' Γ τs α f Γm1 Γm2 (local{τ} s1) (local{τ} s2) (c,mσ)
  | SComp_refine s1 s2 s1' s2' c1 mσ1 c2 mσ2 mσ :
     stmt_refine' Γ τs α f Γm1 Γm2 s1 s2 (c1,mσ1) →
     stmt_refine' Γ τs α f Γm1 Γm2 s1' s2' (c2,mσ2) →
     rettype_union mσ1 mσ2 = Some mσ →
     stmt_refine' Γ τs α f Γm1 Γm2 (s1 ;; s1') (s2 ;; s2') (c2,mσ)
  | SCatch_refine s1 s2 c mσ :
     stmt_refine' Γ τs α f Γm1 Γm2 s1 s2 (c,mσ) →
     stmt_refine' Γ τs α f Γm1 Γm2 (catch s1) (catch s2) (false,mσ)
  | SLoop_refine s1 s2 c mσ :
     stmt_refine' Γ τs α f Γm1 Γm2 s1 s2 (c,mσ) →
     stmt_refine' Γ τs α f Γm1 Γm2 (loop s1) (loop s2) (true,mσ)
  | SIf_refine e1 e2 τb s1 s2 s1' s2' c1 mσ1 c2 mσ2 mσ :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr (baseT τb) →
     stmt_refine' Γ τs α f Γm1 Γm2 s1 s2 (c1,mσ1) →
     stmt_refine' Γ τs α f Γm1 Γm2 s1' s2' (c2,mσ2) →
     rettype_union mσ1 mσ2 = Some mσ →
     stmt_refine' Γ τs α f Γm1 Γm2
...
Inductive sctx_item_refine' (Γ : env Ti) (τs: list (type Ti))
  ...
  | CCompL_refine s1' s2' c mσ c' mσ' mσr :
     s1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2' : (c',mσ') →
     rettype_union mσ mσ' = Some mσr →
     sctx_item_refine' Γ τs α f Γm1 Γm2 (□ ;; s1') (□ ;; s2') (c,mσ) (c',mσr)
  | CCompR_refine s1 s2 c mσ c' mσ' mσr :
     s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : (c,mσ) →
     rettype_union mσ mσ' = Some mσr →
     sctx_item_refine' Γ τs α f Γm1 Γm2 (s1 ;; □) (s2 ;; □) (c',mσ') (c',mσr)
  | Ccatch_refine c mσ :
     sctx_item_refine' Γ τs α f Γm1 Γm2
       (catch □) (catch □) (c,mσ) (false,mσ)
  | CWhile_refine c mσ :
     sctx_item_refine' Γ τs α f Γm1 Γm2 (loop □) (loop □) (c,mσ) (true,mσ)
...
Inductive esctx_item_refine' (Γ : env Ti) (τs: list (type Ti))
  ...
  | CDoE_refine τ :
     esctx_item_refine' Γ τs α f Γm1 Γm2 (! □) (! □) τ (false,None)
  | CReturnE_refine τ :
     esctx_item_refine' Γ τs α f Γm1 Γm2 (ret □) (ret □) τ (true,Some τ)
  | CIfE_refine τb s1 s2 s1' s2' c mσ c' mσ' mσr :
     s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : (c,mσ) →
     s1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2' : (c',mσ') →
     rettype_union mσ mσ' = Some mσr →
     esctx_item_refine' Γ τs α f Γm1 Γm2
...
Inductive ctx_item_refine' (Γ : env Ti) (τs: list (type Ti))
  ...
  | CStmt_refine Es1 Es2 cmσ cmσ' :
     Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : cmσ ↣ cmσ' →
     ctx_item_refine' Γ τs α f Γm1 Γm2
  ...
  | CParams_refine g os1 os2 σs cmσ σ :
     ...
     rettype_match cmσ σ →
     ctx_item_refine' Γ τs α f Γm1 Γm2 (CParams g (zip os1 σs))
...
Inductive ctx_refine' (Γ : env Ti) (α : bool)
  ...
  | ctx_nil_refine_2 τf : ctx_refine' Γ α f Γm1 Γm2 [] [] τf τf
  | ctx_cons_refine_2 Ek1 Ek2 k1 k2 τf τf' τf'' :
     Ek1 ⊑{(Γ,get_stack_types k1),α,f@Γm1↦Γm2} Ek2 : τf ↣ τf' →
     ctx_refine' Γ α f Γm1 Γm2 k1 k2 τf' τf'' →
     ctx_refine' Γ α f Γm1 Γm2 (Ek1 :: k1) (Ek2 :: k2) τf τf''.
...
Inductive focus_refine' (Γ : env Ti) (τs : list (type Ti))
  ...
  | Stmt_refine d1 d2 s1 s2 cmσ :
     s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : cmσ →
     d1 ⊑{Γ,α,f@Γm1↦Γm2} d2 : cmσ →
     focus_refine' Γ τs α f Γm1 Γm2 (Stmt d1 s1) (Stmt d2 s2) (Stmt_type cmσ)
  | Expr_refine e1 e2 τ :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr τ →
     focus_refine' Γ τs α f Γm1 Γm2 (Expr e1) (Expr e2) (Expr_type τ)
  | Call_refine g vs1 vs2 σs σ :
     Γ !! g = Some (σs,σ) →
     vs1 ⊑{Γ,α,f@Γm1↦Γm2}* vs2 :* σs →
     focus_refine' Γ τs α f Γm1 Γm2 (Call g vs1) (Call g vs2) (Fun_type g)
  | Return_refine g σs σ v1 v2 :
     Γ !! g = Some (σs, σ) →
     v1 ⊑{Γ,α,f@Γm1↦Γm2} v2 : σ →
     focus_refine' Γ τs α f Γm1 Γm2 (Return g v1) (Return g v2) (Fun_type g)
  | UndefExpr_refine E1 E2 e1 e2 τlr τ :
     e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr →
     E1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} E2 : τlr ↣ inr τ →
     focus_refine' Γ τs α f Γm1 Γm2
       (Undef (UndefExpr E1 e1)) (Undef (UndefExpr E2 e2)) (Expr_type τ)
  | UndefBranch_refine Es1 Es2 Ω1 Ω2 v1 v2 τ mσ :
     v1 ⊑{Γ,α,f@Γm1↦Γm2} v2 : τ →
     Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : τ ↣ mσ → Ω1 ⊑{Γ,α,f@Γm1↦Γm2} Ω2 →
     focus_refine' Γ τs α f Γm1 Γm2 (Undef (UndefBranch Es1 Ω1 v1))
...
Lemma EVal_refine_inv_l Γ α f Γm1 Γm2 τs Ωs1 vs1 es2 σs :
  #{Ωs1}* vs1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* inr <$> σs →
...
Qed.
Lemma EVal_refine_inv_r Γ α f Γm1 Γm2 τs es1 Ωs2 vs2 σs :
  es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* #{Ωs2}* vs2 :* inr <$> σs →
...
Proof.
  ...
  eexists (_ :: _), (_ :: _);
    split_ands; simpl; eauto using Forall3_cons with arith.
Qed.
Lemma ectx_item_subst_refine Γ α f Γm1 Γm2 τs Ei1 Ei2 e1 e2 τlr τlr' :
  Ei1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ei2 : τlr ↣ τlr' →
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr →
  subst Ei1 e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst Ei2 e2 : τlr'.
Proof.
  destruct 1; simpl; refine_constructor; eauto.
  rewrite ?fmap_app; eauto using Forall3_app, Forall3_cons.
Qed.
Lemma ectx_subst_refine Γ α f Γm1 Γm2 τs E1 E2 e1 e2 τlr τlr' :
  E1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} E2 : τlr ↣ τlr' →
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr →
  subst E1 e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst E2 e2 : τlr'.
Proof.
  intros HE. revert e1 e2.
  induction HE; simpl; eauto using ectx_item_subst_refine.
Qed.
Lemma sctx_item_subst_refine Γ α f Γm1 Γm2 τs Es1 Es2 s1 s2 mcτ mcτ' :
  Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ →
  subst Es1 s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst Es2 s2 : mcτ'.
Proof. destruct 1; simpl; refine_constructor; eauto. Qed.
Lemma ectx_item_subst_refine_inv_r Γ α f Γm1 Γm2 τs e1 Ei2 e2 τlr' :
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst Ei2 e2 : τlr' →
...
Qed.
Lemma ectx_subst_refine_inv_r Γ α f Γm1 Γm2 τs e1 E2 e2 τlr' :
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst E2 e2 : τlr' →
...
Proof.
  ...
  destruct (ectx_item_subst_refine_inv_r Γ α f Γm1 Γm2 τs e1' Ei2 e2 τlr)
    as (e1&Ee1&τlr''&->&?&?); auto.
  exists e1 (Ee1 :: E1) τlr''; split_ands; repeat refine_constructor; eauto.
Qed.
Lemma esctx_item_subst_refine_inv_r Γ α f Γm1 Γm2 τs s1 Ee2 e2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst Ee2 e2 : mcτ →
  ∃ e1' Ee1 τ, s1 = subst Ee1 e1' ∧
    Ee1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ee2 : τ ↣ mcτ ∧
    e1' ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : inr τ.
Proof.
  by destruct Ee2; inversion 1; subst;
    do 3 eexists; split_ands; repeat refine_constructor; eauto.
Qed.
Lemma sctx_item_subst_refine_inv_r Γ α f Γm1 Γm2 τs s1 Es2 s2 mcτ' :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} subst Es2 s2 : mcτ' →
...
Lemma expr_refine_nf_inv Γ α f Γm1 Γm2 τs e1 e2 τlr :
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr → is_nf e2 → is_nf e1.
Proof. destruct 1; inversion_clear 1; constructor. Qed.
Lemma exprs_refine_nf_inv Γ α f Γm1 Γm2 τs es1 es2 τlrs :
  es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* τlrs →
  Forall is_nf es2 → Forall is_nf es1.
Proof. induction 1; inversion_clear 1; eauto using expr_refine_nf_inv. Qed.
Lemma expr_refine_redex_inv Γ α f Γm1 Γm2 τs e1 e2 τlr :
  e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr → is_redex e2 → is_redex e1.
Proof.
  destruct 1; inversion_clear 1;
    constructor; eauto using expr_refine_nf_inv, exprs_refine_nf_inv.
Qed.
Lemma stmt_refine_gotos Γ α f Γm1 Γm2 τs s1 s2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → gotos s1 = gotos s2.
Proof. induction 1; simpl; auto with f_equal. Qed.
Lemma stmt_refine_labels Γ α f Γm1 Γm2 τs s1 s2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → labels s1 = labels s2.
Proof. induction 1; simpl; auto with f_equal. Qed.
Lemma stmt_refine_labels_elem_of_r Γ α f Γm1 Γm2 τs s1 s2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → labels s1 = labels s2.
Proof. induction 1; simpl; auto with f_equal. Qed.
Lemma stmt_refine_throws_valid Γ α f Γm1 Γm2 τs s1 s2 mcτ n :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → throws_valid n s1 → throws_valid n s2.
Proof. intros Hs. revert n. induction Hs; naive_solver. Qed.
Lemma ctx_refine_stack_types Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' →
...
Qed.
Lemma ctx_refine_stack Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' →
...
Qed.
Lemma sctx_item_catch_refine Γ α f Γm1 Γm2 τs Es1 Es2 s1 s2 mcτ mcτ' :
  Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  Es1 = catch □ → Es2 = catch □.
Proof. by destruct 1. Qed.  
Lemma direction_in_refine_r Γ τs α f Γm1 Γm2 s1 s2 d1 d2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ →
...
Lemma direction_out_refine_r Γ τs α f Γm1 Γm2 s1 s2 d1 d2 mcτ :
  s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ →
  d1 ⊑{Γ,α,f@Γm1↦Γm2} d2 : mcτ → direction_out d2 s2 → direction_out d1 s1.
Proof. by destruct 2; simpl; erewrite <-?stmt_refine_labels by eauto. Qed.
Lemma funenv_lookup_refine_r Γ α f Γm1 Γm2 δ1 δ2 g s2 :
  δ1 ⊑{Γ,α,f@Γm1↦Γm2} δ2 → δ2 !! g = Some s2 → ∃ s1 τs τ cmτ,
...
Qed.
Lemma expr_refine_typed_l Γ α f Γm1 Γm2 τs e1 e2 τlr :
  ✓ Γ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr → (Γ,Γm1,τs) ⊢ e1 : τlr.
Proof.
  assert (∀ es1 es2 τlrs,
    Forall3 (λ e1 _ τlr, (Γ,Γm1,τs) ⊢ e1 : τlr) es1 es2 τlrs →
    (Γ,Γm1,τs) ⊢* es1 :* τlrs).
...
Qed.
Lemma exprs_refine_typed_l Γ α f Γm1 Γm2 τs es1 es2 τlrs :
  ✓ Γ → es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* τlrs →
  (Γ,Γm1,τs) ⊢* es1 :* τlrs.
Proof. induction 2; eauto using expr_refine_typed_l. Qed.
Lemma ectx_item_refine_typed_l Γ α f Γm1 Γm2 τs Ei1 Ei2 τlr τlr' :
  ✓ Γ → Ei1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ei2 : τlr ↣ τlr' →
  (Γ,Γm1,τs) ⊢ Ei1 : τlr ↣ τlr'.
Proof.
  destruct 2; typed_constructor;
    eauto using expr_refine_typed_l, exprs_refine_typed_l.
Qed.
Lemma ectx_refine_typed_l Γ α f Γm1 Γm2 τs E1 E2 τlr τlr' :
  ✓ Γ → E1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} E2 : τlr ↣ τlr' →
  (Γ,Γm1,τs) ⊢ E1 : τlr ↣ τlr'.
Proof. induction 2;typed_constructor; eauto using ectx_item_refine_typed_l. Qed.
Lemma stmt_refine_typed_l Γ α f Γm1 Γm2 τs s1 s2 mcτ :
  ✓ Γ → s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → (Γ,Γm1,τs) ⊢ s1 : mcτ.
Proof. induction 2; typed_constructor; eauto using expr_refine_typed_l. Qed.
Lemma sctx_item_refine_typed_l Γ α f Γm1 Γm2 τs Es1 Es2 mcτ mcτ' :
  ✓ Γ → Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  (Γ,Γm1,τs) ⊢ Es1 : mcτ ↣ mcτ'.
Proof.
  destruct 2; typed_constructor;
    eauto using stmt_refine_typed_l, expr_refine_typed_l.
Qed.
Lemma esctx_item_refine_typed_l Γ α f Γm1 Γm2 τs Ee1 Ee2 τlr mcτ :
  ✓ Γ → Ee1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ee2 : τlr ↣ mcτ →
  (Γ,Γm1,τs) ⊢ Ee1 : τlr ↣ mcτ.
Proof. destruct 2; typed_constructor; eauto using stmt_refine_typed_l. Qed.
Lemma ctx_item_refine_typed_l Γ α f Γm1 Γm2 τs Ek1 Ek2 τf τf' :
  ✓ Γ → Ek1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ek2 : τf ↣ τf' →
  (Γ,Γm1,τs) ⊢ Ek1 : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_typed_l Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' → (Γ,Γm1) ⊢ k1 : τf ↣ τf'.
...
Lemma focus_refine_typed_l Γ α f Γm1 Γm2 τs φ1 φ2 τf :
  ✓ Γ → φ1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} φ2 : τf → (Γ,Γm1,τs) ⊢ φ1 : τf.
Proof.
...
Qed.
Lemma funenv_refine_typed_l Γ α f Γm1 Γm2 δ1 δ2 :
  ✓ Γ → δ1 ⊑{Γ,α,f@Γm1↦Γm2} δ2 → ✓{Γ,Γm1} δ1.
Proof.
  intros ? Hδ; split.
...
Qed.
Lemma expr_refine_typed_r Γ α f Γm1 Γm2 τs e1 e2 τlr :
  ✓ Γ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr → (Γ,Γm2,τs) ⊢ e2 : τlr.
Proof.
  assert (∀ es1 es2 τlrs,
    Forall3 (λ _ e2 τlr, (Γ,Γm2,τs) ⊢ e2 : τlr) es1 es2 τlrs →
    (Γ,Γm2,τs) ⊢* es2 :* τlrs).
  { induction 1; constructor; eauto. }
  induction 2 using @expr_refine_ind; typed_constructor;
...
Qed.
Lemma exprs_refine_typed_r Γ α f Γm1 Γm2 τs es1 es2 τlrs :
  ✓ Γ → es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* τlrs →(Γ,Γm2,τs) ⊢* es2 :* τlrs.
Proof. induction 2; eauto using expr_refine_typed_r. Qed.
Lemma ectx_item_refine_typed_r Γ α f Γm1 Γm2 τs Ei1 Ei2 τlr τlr' :
  ✓ Γ → Ei1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ei2 : τlr ↣ τlr' →
  (Γ,Γm2,τs) ⊢ Ei2 : τlr ↣ τlr'.
Proof.
  destruct 2; typed_constructor;
    eauto using expr_refine_typed_r, exprs_refine_typed_r.
Qed.
Lemma ectx_refine_typed_r Γ α f Γm1 Γm2 τs E1 E2 τlr τlr' :
  ✓ Γ → E1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} E2 : τlr ↣ τlr' →
  (Γ,Γm2,τs) ⊢ E2 : τlr ↣ τlr'.
Proof. induction 2;typed_constructor; eauto using ectx_item_refine_typed_r. Qed.
Lemma stmt_refine_typed_r Γ f α Γm1 Γm2 τs s1 s2 mcτ :
  ✓ Γ → s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → (Γ,Γm2,τs) ⊢ s2 : mcτ.
Proof. induction 2; typed_constructor; eauto using expr_refine_typed_r. Qed.
Lemma sctx_item_refine_typed_r Γ α f Γm1 Γm2 τs Es1 Es2 mcτ mcτ' :
  ✓ Γ → Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  (Γ,Γm2,τs) ⊢ Es2 : mcτ ↣ mcτ'.
Proof.
  destruct 2; typed_constructor;
    eauto using stmt_refine_typed_r, expr_refine_typed_r.
Qed.
Lemma esctx_item_refine_typed_r Γ α f Γm1 Γm2 τs Ee1 Ee2 τlr mcτ :
  ✓ Γ → Ee1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ee2 : τlr ↣ mcτ →
  (Γ,Γm2,τs) ⊢ Ee2 : τlr ↣ mcτ.
Proof. destruct 2; typed_constructor; eauto using stmt_refine_typed_r. Qed.
Lemma ctx_item_refine_typed_r Γ α f Γm1 Γm2 τs Ek1 Ek2 τf τf' :
  ✓ Γ → Ek1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ek2 : τf ↣ τf' →
  (Γ,Γm2,τs) ⊢ Ek2 : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_typed_r Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' → (Γ,Γm2) ⊢ k2 : τf ↣ τf'.
Proof.
...
Lemma focus_refine_typed_r Γ α f Γm1 Γm2 τs φ1 φ2 τf :
  ✓ Γ → φ1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} φ2 : τf → (Γ,Γm2,τs) ⊢ φ2 : τf.
Proof.
...
Qed.
Lemma funenv_refine_typed_r Γ α f Γm1 Γm2 δ1 δ2 :
  ✓ Γ → δ1 ⊑{Γ,α,f@Γm1↦Γm2} δ2 → ✓{Γ,Γm2} δ2.
Proof.
  intros ? Hδ; split.
...
Lemma expr_refine_id Γ α Γm τs e τlr :
  (Γ,Γm,τs) ⊢ e : τlr → e ⊑{(Γ,τs),α@Γm} e : τlr.
Proof.
  assert (∀ es τlrs, Forall2 (λ e τlr, e ⊑{(Γ,τs),α@Γm} e : τlr) es τlrs →
    es ⊑{(Γ,τs),α@Γm}* es :* τlrs).
  { induction 1; constructor; eauto. }
  induction 1 using @expr_typed_ind; refine_constructor; eauto using
    locks_refine_id, val_refine_id, addr_refine_id.
Qed.
Lemma exprs_refine_id Γ α Γm τs es τlrs :
  (Γ,Γm,τs) ⊢* es :* τlrs → es ⊑{(Γ,τs),α@Γm}* es :* τlrs.
Proof. induction 1; constructor; eauto using expr_refine_id. Qed.  
Lemma ectx_item_refine_id Γ α Γm τs Ei τlr τlr' :
  (Γ,Γm,τs) ⊢ Ei : τlr ↣ τlr' → Ei ⊑{(Γ,τs),α@Γm} Ei : τlr ↣ τlr'.
Proof.
  destruct 1; refine_constructor; eauto using expr_refine_id, exprs_refine_id.
Qed.
Lemma ectx_refine_id Γ α Γm τs E τlr τlr' :
  (Γ,Γm,τs) ⊢ E : τlr ↣ τlr' → E ⊑{(Γ,τs),α@Γm} E : τlr ↣ τlr'.
Proof. induction 1; refine_constructor; eauto using ectx_item_refine_id. Qed.
Lemma stmt_refine_id Γ α Γm τs s mcσ :
  (Γ,Γm,τs) ⊢ s : mcσ → s ⊑{(Γ,τs),α@Γm} s : mcσ.
Proof. induction 1; refine_constructor; eauto using expr_refine_id. Qed.
Lemma sctx_item_refine_id Γ α Γm τs Es mcσ mcσ' :
  (Γ,Γm,τs) ⊢ Es : mcσ ↣ mcσ' → Es ⊑{(Γ,τs),α@Γm} Es : mcσ ↣ mcσ'.
Proof.
  destruct 1; refine_constructor; eauto using stmt_refine_id, expr_refine_id.
Qed.
Lemma esctx_item_refine_id Γ α Γm τs Ee τlr τlr' :
  (Γ,Γm,τs) ⊢ Ee : τlr ↣ τlr' → Ee ⊑{(Γ,τs),α@Γm} Ee : τlr ↣ τlr'.
Proof. destruct 1; refine_constructor; eauto using stmt_refine_id. Qed.
Lemma ctx_item_refine_id Γ α Γm τs Ek τf τf' :
  (Γ,Γm,τs) ⊢ Ek : τf ↣ τf' → Ek ⊑{(Γ,τs),α@Γm} Ek : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_id Γ α Γm k τf τf' :
  (Γ,Γm) ⊢ k : τf ↣ τf' → k ⊑{Γ,α@Γm} k : τf ↣ τf'.
...
Lemma focus_refine_id Γ α Γm τs φ τf :
  (Γ,Γm,τs) ⊢ φ : τf → φ ⊑{(Γ,τs),α@Γm} φ : τf.
Proof.
...
Qed.
Lemma funenv_refine_id Γ α Γm δ : ✓ Γ → ✓{Γ,Γm} δ → δ ⊑{Γ,α@Γm} δ.
Proof.
...
Qed.
Lemma expr_refine_compose Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 e1 e2 e3 τlr τlr' :
  ✓ Γ → e1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} e2 : τlr →
  e2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} e3 : τlr' →
  e1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} e3 : τlr.
Proof.
  assert (∀ es1 es2 es3 τlrs τlrs',
    Forall3 (λ e1 e2 τlr, ∀ e3 τlr', e2 ⊑{(Γ,τs),α2,f2@ Γm2↦Γm3} e3 : τlr' →
      e1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1 @ Γm1↦Γm3} e3 : τlr) es1 es2 τlrs →
    es2 ⊑{(Γ,τs),α2,f2@ Γm2↦Γm3}* es3 :* τlrs' →
    es1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1 @ Γm1↦Γm3}* es3 :* τlrs).
...
Lemma exprs_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 es1 es2 es3 τlrs τlrs' :
  ✓ Γ → es1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2}* es2 :* τlrs →
  es2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3}* es3 :* τlrs' →
  es1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3}* es3 :* τlrs.
Proof.
...
Lemma ectx_item_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 Ei1 Ei2 Ei3 τlr τlr' τlr'' :
  ✓ Γ → Ei1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} Ei2 : τlr ↣ τlr' →
  Ei2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} Ei3 : τlr ↣ τlr'' →
  Ei1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} Ei3 : τlr ↣ τlr'.
Proof.
...
Lemma ectx_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 E1 E2 E3 τlr τlr' τlr'' :
  ✓ Γ → E1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} E2 : τlr ↣ τlr' →
  E2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} E3 : τlr ↣ τlr'' →
  E1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} E3 : τlr ↣ τlr'.
Proof.
  intros ? HE. revert E3 τlr''.
...
Qed.
Lemma stmt_refine_compose Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 s1 s2 s3 mcτ mcτ' :
  ✓ Γ → s1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} s2 : mcτ →
  s2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} s3 : mcτ' →
  s1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} s3 : mcτ.
Proof.
...
Lemma sctx_item_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 Es1 Es2 Es3 mcτ mcτ' mcτ'' :
  ✓ Γ → Es1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  Es2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} Es3 : mcτ ↣ mcτ'' →
  Es1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} Es3 : mcτ ↣ mcτ'.
Proof.
...
Lemma esctx_item_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 Ee1 Ee2 Ee3 τlr mcτ' mcτ'' :
  ✓ Γ → Ee1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} Ee2 : τlr ↣ mcτ' →
  Ee2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} Ee3 : τlr ↣ mcτ'' →
  Ee1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} Ee3 : τlr ↣ mcτ'.
Proof.
...
Lemma ctx_item_refine_compose
    Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 Ek1 Ek2 Ek3 τf τf' τf'' :
  ✓ Γ → Ek1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} Ek2 : τf ↣ τf' →
  Ek2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} Ek3 : τf ↣ τf'' →
  Ek1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} Ek3 : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_compose Γ α1 α2 f1 f2 Γm1 Γm2 Γm3 k1 k2 k3 τf τf' τf'' :
  ✓ Γ → k1 ⊑{Γ,α1,f1@Γm1↦Γm2} k2 : τf ↣ τf' →
  k2 ⊑{Γ,α2,f2@Γm2↦Γm3} k3 : τf ↣ τf'' →
  k1 ⊑{Γ,α1||α2,f2 ◎ f1@Γm1↦Γm3} k3 : τf ↣ τf'.
Proof.
...
Qed.
Lemma focus_refine_compose Γ τs α1 α2 f1 f2 Γm1 Γm2 Γm3 φ1 φ2 φ3 τf τf' :
  ✓ Γ → φ1 ⊑{(Γ,τs),α1,f1@Γm1↦Γm2} φ2 : τf →
  φ2 ⊑{(Γ,τs),α2,f2@Γm2↦Γm3} φ3 : τf' →
  φ1 ⊑{(Γ,τs),α1||α2,f2 ◎ f1@Γm1↦Γm3} φ3 : τf.
Proof.
...
Qed.
Lemma funenv_refine_compose Γ α1 α2 f1 f2 Γm1 Γm2 Γm3 δ1 δ2 δ3 :
  ✓ Γ → δ1 ⊑{Γ,α1,f1@Γm1↦Γm2} δ2 → δ2 ⊑{Γ,α2,f2@Γm2↦Γm3} δ3 →
  δ1 ⊑{Γ,α1||α2,f2 ◎ f1@Γm1↦Γm3} δ3.
Proof.
...
Qed.
Lemma expr_refine_inverse Γ τs f Γm1 Γm2 e1 e2 τlr :
  e1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} e2 : τlr →
  e2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} e1 : τlr.
Proof.
  assert (∀ es1 es2 τlrs,
    Forall3 (λ e1 e2 τlr,
      e2 ⊑{(Γ,τs),false,meminj_inverse f@ Γm2↦Γm1} e1 : τlr) es1 es2 τlrs →
    es2 ⊑{(Γ,τs),false,meminj_inverse f@ Γm2↦Γm1}* es1 :* τlrs).
  { induction 1; constructor; eauto. }
...
Qed.
Lemma exprs_refine_inverse Γ τs f Γm1 Γm2 es1 es2 τlrs :
  es1 ⊑{(Γ,τs),false,f@Γm1↦Γm2}* es2 :* τlrs →
  es2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1}* es1 :* τlrs.
Proof. induction 1; constructor; eauto using expr_refine_inverse. Qed.
Lemma ectx_item_refine_inverse Γ τs f Γm1 Γm2 Ei1 Ei2 τlr τlr' :
  Ei1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} Ei2 : τlr ↣ τlr' →
  Ei2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} Ei1 : τlr ↣ τlr'.
Proof.
  destruct 1; refine_constructor;
    eauto using expr_refine_inverse, exprs_refine_inverse.
Qed.
Lemma ectx_refine_inverse Γ τs f Γm1 Γm2 E1 E2 τlr τlr' :
  E1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} E2 : τlr ↣ τlr' →
  E2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} E1 : τlr ↣ τlr'.
Proof. induction 1; econstructor; eauto using ectx_item_refine_inverse. Qed.
Lemma stmt_refine_inverse Γ τs f Γm1 Γm2 s1 s2 mcτ :
  s1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} s2 : mcτ →
  s2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} s1 : mcτ.
Proof.
  remember false as α. induction 1; subst;
    refine_constructor; naive_solver eauto using expr_refine_inverse.
Qed.
Lemma sctx_item_refine_inverse Γ τs f Γm1 Γm2 Es1 Es2 mcτ mcτ' :
  Es1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' →
  Es2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} Es1 : mcτ ↣ mcτ'.
Proof.
  destruct 1; refine_constructor;
    eauto using stmt_refine_inverse, expr_refine_inverse.
Qed.
Lemma esctx_item_refine_inverse Γ τs f Γm1 Γm2 Ee1 Ee2 τlr mcτ' :
  Ee1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} Ee2 : τlr ↣ mcτ' →
  Ee2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} Ee1 : τlr ↣ mcτ'.
Proof. destruct 1; refine_constructor; eauto using stmt_refine_inverse. Qed.
Lemma ctx_item_refine_inverse Γ τs f Γm1 Γm2 Ek1 Ek2 τf τf' :
  Γm1 ⊑{Γ,false,f} Γm2 → Ek1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} Ek2 : τf ↣ τf' →
  Ek2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} Ek1 : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_inverse Γ f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → Γm1 ⊑{Γ,false,f} Γm2 → k1 ⊑{Γ,false,f@Γm1↦Γm2} k2 : τf ↣ τf' →
  k2 ⊑{Γ,false,meminj_inverse f@Γm2↦Γm1} k1 : τf ↣ τf'.
Proof.
...
Lemma focus_refine_inverse Γ τs f Γm1 Γm2 φ1 φ2 τf :
  φ1 ⊑{(Γ,τs),false,f@Γm1↦Γm2} φ2 : τf →
  φ2 ⊑{(Γ,τs),false,meminj_inverse f@Γm2↦Γm1} φ1 : τf.
Proof.
...
Qed.
Lemma funenv_refine_inverse Γ f Γm1 Γm2 δ1 δ2 :
  δ1 ⊑{Γ,false,f@Γm1↦Γm2} δ2 →
  δ2 ⊑{Γ,false,meminj_inverse f@Γm2↦Γm1} δ1.
Proof.
...
Lemma expr_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs e1 e2 τlr :
  ✓ Γ → e1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} e2 : τlr → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → e1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} e2 : τlr.
Proof.
...
Qed.
Lemma exprs_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs es1 es2 τlrs :
  ✓ Γ → es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2}* es2 :* τlrs → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 →
  es1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'}* es2 :* τlrs.
Proof. induction 2; constructor; eauto using expr_refine_weaken. Qed.
Lemma ectx_item_refine_weaken
    Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs Ei1 Ei2 τlr τlr' :
  ✓ Γ → Ei1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ei2 : τlr ↣ τlr' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 →
  Ei1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} Ei2 : τlr ↣ τlr'.
Proof.
...
Qed.
Lemma ectx_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs E1 E2 τlr τlr' :
  ✓ Γ → E1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} E2 : τlr ↣ τlr' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → E1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} E2 : τlr ↣ τlr'.
Proof. induction 2; refine_constructor;eauto using ectx_item_refine_weaken. Qed.
Lemma stmt_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs s1 s2 mcτ :
  ✓ Γ → s1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} s2 : mcτ → (α → α') → Γm1' ⊑{Γ,α',f'} Γm2' →
...
Qed.
Lemma sctx_item_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs Es1 Es2 mcτ mcτ' :
  ✓ Γ → Es1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Es2 : mcτ ↣ mcτ' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 →
  Es1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} Es2 : mcτ ↣ mcτ'.
Proof.
...
Lemma esctx_item_refine_weaken
    Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs Ee1 Ee2 τ mcτ' :
  ✓ Γ → Ee1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ee2 : τ ↣ mcτ' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → Ee1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} Ee2 : τ ↣ mcτ'.
Proof. destruct 2; refine_constructor; eauto using stmt_refine_weaken. Qed.
Lemma ctx_item_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs Ek1 Ek2 τf τf' :
  ✓ Γ → Ek1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} Ek2 : τf ↣ τf' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → Ek1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} Ek2 : τf ↣ τf'.
Proof.
...
Qed.
Lemma ctx_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → k1 ⊑{Γ,α',f'@Γm1'↦Γm2'} k2 : τf ↣ τf'.
...
Lemma focus_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' τs φ1 φ2 τf :
  ✓ Γ → φ1 ⊑{(Γ,τs),α,f@Γm1↦Γm2} φ2 : τf → (α → α') →
  Γm1' ⊑{Γ,α',f'} Γm2' → Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' →
  meminj_extend f f' Γm1 Γm2 → φ1 ⊑{(Γ,τs),α',f'@Γm1'↦Γm2'} φ2 : τf.
Proof.
...
Qed.
Lemma funenv_refine_weaken Γ α α' f f' Γm1 Γm2 Γm1' Γm2' δ1 δ2 :
  ✓ Γ → δ1 ⊑{Γ,α,f@Γm1↦Γm2} δ2 → (α → α') → Γm1' ⊑{Γ,α',f'} Γm2' →
  Γm1 ⇒ₘ Γm1' → Γm2 ⇒ₘ Γm2' → meminj_extend f f' Γm1 Γm2 →
  δ1 ⊑{Γ,α',f'@Γm1'↦Γm2'} δ2.
Proof.
  intros ? Hδ ????? h; specialize (Hδ h); destruct (δ1 !! h) as [s1|],
...
Lemma ctx_refine_stack_typed_l Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' →
  Γm1 ⊢* get_stack k1 :* get_stack_types k1.
Proof. eauto using ctx_typed_stack_typed, ctx_refine_typed_l. Qed.
Lemma ctx_refine_stack_typed_r Γ α f Γm1 Γm2 k1 k2 τf τf' :
  ✓ Γ → k1 ⊑{Γ,α,f@Γm1↦Γm2} k2 : τf ↣ τf' →
