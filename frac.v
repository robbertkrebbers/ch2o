...
Bind Scope frac_scope with frac.
Delimit Scope frac_scope with frac.
...
Notation "1" := frac1 : frac_scope.

Definition frac_gen_kind (k : pkind) (f : frac) : pkind :=
  if decide (f = 1%frac) then k else Read.
Instance frac_kind: PermKind frac := frac_gen_kind Write.

Instance frac_lock: PermLock frac := λ _ f, f.

Instance frac_subseteq: SubsetEq frac := λ f1 f2, `f1 ≤ `f2.
Instance frac_subseteq_dec (f1 f2 : frac) :
  Decision (f1 ⊆ f2) := decide (`f1 ≤ `f2).

Lemma frac_subset_spec f1 f2 : f1 ⊂ f2 ↔ `f1 < `f2.
Proof.
  unfold subset, orders.preorder_subset, subseteq, frac_subseteq.
  rewrite <-Qclt_nge. intuition auto using Qclt_le_weak.
Qed.

Instance frac_disjoint: Disjoint frac := λ f1 f2, `f1 + `f2 ≤ 1.
Instance frac_disjoint_dec (f1 f2 : frac) :
  Decision (f1 ⊥ f2) := decide (`f1 + `f2 ≤ 1).
...
Proof.
  intros ??. unfold disjoint, frac_disjoint.
  by rewrite Qcplus_comm.
Qed.

Lemma frac_disjoint_weaken_l f1 f1' f2 :
  f1 ⊥ f2 →
  f1' ⊆ f1 →
  f1' ⊥ f2.
Proof.
  unfold disjoint, frac_disjoint, subseteq, frac_subseteq.
  intros ??. transitivity (`f1 + `f2); [|done].
  by apply Qcplus_le_mono_r.
Qed.

Lemma frac_union_not_disjoint f1 f2 :
  ¬f1 ⊥ f2 →
  f1 ∪ f2 = 1%frac.
Proof. intros. unfold union, frac_union. by case_decide. Qed.
Lemma frac_union_disjoint f1 f2 :
  f1 ⊥ f2 →
...
Lemma frac_disjoint_union_ll f1 f2 f3 :
  f1 ∪ f2 ⊥ f3 → f1 ⊥ f3.
Proof.
  unfold union, frac_union, disjoint, frac_disjoint; simpl.
  case_decide; simpl.
...
Proof.
  ...
  * intros f1 f2 f3 H1. unfold subseteq, frac_subseteq.
    ...
    + rewrite !(frac_union_not_disjoint _ f2) by done.
      ...
      by rewrite frac_union_not_disjoint.
  * intros f1 f2 f3 H1 H2 H3. unfold subseteq, frac_subseteq in *.
  ...
  * intros f1 f2 H1. do 2 red. rewrite frac_subset_spec in H1.
    unfold difference, frac_difference. case_decide; simpl; [|done].
...
Qed.
Notation "f ↓" := (frac_split f) (at level 20) : frac_scope.

Lemma frac_split_disjoint f : (f↓ ⊥ f↓)%frac.
Proof.
...
Qed.
Lemma frac_split_union f : (f↓ ∪ f↓ = f)%frac.
Proof.
  apply dsig_eq. rewrite frac_union_disjoint by auto using frac_split_disjoint.
