...
Lemma ehstep_exec_complete Γ Γf m1 m2 ρ τs e1 e2 τlr :
  ...
  (**i 3.) *) m2' ⊑{Γ,f} m2 ∧
  (**i 4.) *) (∀ o τ, '{m1} ⊢ o : τ → '{m2} ⊢ o : τ) ∧
  (**i 5.) *) (∀ o τ, '{m1} ⊢ o : τ → '{m2'} ⊢ o : τ) ∧
  (**i 6.) *) (∀ o τ, '{m1} ⊢ o : τ → f !! o = Some (o,[])) ∧
  (**i 7.) *) (∀ o τ,
    '{m1} ⊢ o : τ → index_alive ('{m2'}) o → index_alive ('{m1}) o) ∧
  (**i 8.) *) (∀ o o' r τ, '{m1} ⊢ o' : τ →
    f !! o = Some (o',r) → mem_inj_id !! o = Some (o',r)).
Proof.
  intros ? Hm1 Hρ He1 p. destruct (ehstep_preservation Γ Γf m1 m2 ρ
    τs e1 e2 τlr) as (Hm2&He2&Hotyped&Hoalive); auto.
  revert Hm1 Hρ He1 Hm2 He2 Hotyped Hoalive. case p; clear p; try by (
  ...
  refine_constructor; eauto using lockset_valid_weaken, mem_alloc_index_typed.
  * eapply locks_refine_weaken with mem_inj_id ('{m}) ('{m});
      eauto using locks_refine_id,
      mem_alloc_index_typed, mem_alloc_index_alive_inv, option_eq_1.
  * eapply addr_top_array_refine; eauto using mem_alloc_index_typed_eq.
  * eauto using mem_alloc_index_typed.
  * eauto using mem_alloc_index_alive_inv.
Qed.
...
Lemma cstep_exec_complete Γ Γf δ S1 S2 g :
  ...
  ∃ f S2', S2' ∈ cstep_exec Γ δ S1 ∧ S2' ⊑{(Γ,Γf),f} S2 : g.
Proof.
  intros ? Hδ1 HS1 p.
  ...
  * intros m1 m2 k E e1 e2 ?? (τlr&Heτlr&?&?) _ _; simpl in *.
    ...
    exists f (State k (Expr (subst E e2')) m2'); split.
    ...
    eexists; split_ands; repeat refine_constructor; eauto.
    + eapply ectx_subst_refine; eauto.
      eapply ectx_refine_weaken; eauto using ectx_refine_id.
    + eapply ctx_refine_weaken; eauto using ctx_refine_id.
  ...
  * intros m k h s os vs ???? (τf&?&?&?) _ _; simpl in *; typed_inversion_all.
    ...
    split; [unfold of_option; simplify_option_equality; esolve_elem_of|].
    erewrite fmap_type_of by eauto.
    eexists; split_ands; simpl; repeat refine_constructor;
      eauto using mem_alloc_val_list_index_typed_eq.
    + rewrite snd_zip by solve_length.
      ...
      eapply (stmt_refine_weaken _ _ mem_inj_id _ ('{m}));
        eauto using stmt_refine_id, mem_alloc_val_list_index_typed,
        mem_alloc_val_list_index_alive_inv.
    + eapply (ctx_refine_weaken _ _ mem_inj_id _ ('{m}));
        eauto using ctx_refine_id, mem_alloc_val_list_index_typed,
        mem_alloc_val_list_index_alive_inv.
  ...
  * intros m k d o τ s ??? (τf&?&?&?) _ _; simpl in *; typed_inversion_all.
    ...
    split; [by destruct d; simplify_option_equality; try case_match; eauto|].
    eexists; split_ands; simpl; repeat refine_constructor;
      eauto using mem_alloc_index_typed_eq.
    + eapply (stmt_refine_weaken _ _ mem_inj_id _ ('{m})); eauto using
        stmt_refine_id, mem_alloc_index_typed, mem_alloc_index_alive_inv.
    + eapply (direction_refine_weaken _ mem_inj_id _ ('{m})); eauto using
        direction_refine_id, mem_alloc_index_typed,
        mem_alloc_index_alive_inv, option_eq_1_alt.
    + eapply (ctx_refine_weaken _ _ mem_inj_id _ ('{m})); eauto using
        ctx_refine_id, mem_alloc_index_typed, mem_alloc_index_alive_inv.
  * intros m k d o τ s ?????.
    eexists mem_inj_id, _; split_ands; eauto using state_refine_id.
