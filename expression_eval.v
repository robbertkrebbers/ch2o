...
Require Export fin_map_dom expressions abstract_memory values_old.
...
Lemma mapM_expr_eval_val δ ρ m Ωs vs :
  Ωs `same_length` vs →
  ⋃ Ωs = ∅ → mapM (λ e, ⟦ e ⟧ δ ρ m) (zip_with EVal Ωs vs) = Some vs.
Proof.
...
Lemma Forall2_expr_eval_val_inv δ ρ m Ωs vs ws:
  Ωs `same_length` vs →
  Forall2 (λ e v, ⟦ e ⟧ δ ρ m = Some v) (zip_with EVal Ωs vs) ws → vs = ws.
Proof.
  intros HΩvs. revert ws. induction HΩvs; simpl; intros; decompose_Forall;
...
behavior. *)
Lemma expr_eval_is_pure δ ρ m e v : ⟦ e ⟧ δ ρ m = Some v → is_pure (dom _ δ) e.
Proof.
...
Lemma expr_eval_weaken_mem δ ρ m1 m2 e v :
  ⟦ e ⟧ δ ρ m1 = Some v → m1 ⊆ m2 → ⟦ e ⟧ δ ρ m2 = Some v.
Proof. eauto using expr_eval_weaken_mem_lookup, mem_lookup_weaken. Qed.

Lemma expr_eval_weaken_inv δ ρ m1 m2 e v1 v2 :
  ⟦ e ⟧ δ ρ m1 = Some v1 → m1 ⊆ m2 → ⟦ e ⟧ δ ρ m2 = Some v2 → v1 = v2.
Proof.
  intros ? Hm1 Hm2.
  erewrite (expr_eval_weaken_mem _ _ m1 m2) in Hm2 by eauto. congruence.
Qed.
...
Lemma expr_eval_disjoint δ ρ m1 m2 e v1 v2 :
  ⊥ [m1; m2] → ⟦ e ⟧ δ ρ m1 = Some v1 → ⟦ e ⟧ δ ρ m2 = Some v2 → v1 = v2.
Proof.
  intros Hm1m2.
  assert (∀ es vs1 vs2,
    Forall (λ e, ∀ w1 w2,
...
Lemma expr_eval_fun_irrel δ1 δ2 ρ m e :
  (∀ f, f ∈ funs e → δ1 !! f = δ2 !! f) → ⟦ e ⟧ δ1 ρ m = ⟦ e ⟧ δ2 ρ m.
Proof.
...
Lemma expr_eval_lift δ ρ m e : ⟦ e↑ ⟧ δ ρ m = ⟦ e ⟧ δ (tail ρ) m.
Proof.
...
Lemma expr_eval_subst δ ρ m (E : ectx) e v :
