...
Section pointer_bit_operations.
  ...
  Global Instance ptr_bit_valid :
      Valid (env Ti * option M) (ptr_bit Ti) := λ Γmm pb, ∃ τ,
    let (Γ,mm) := Γmm in
    (Γ,mm) ⊢ frag_item pb : τ ∧
    frozen (frag_item pb) ∧
    frag_index pb < bit_size_of Γ (τ.*).
  Global Instance ptr_bit_valid_dec Γmm (pb : ptr_bit Ti) : Decision (✓{Γmm} pb).
  Proof.
   refine
    match Some_dec (type_check Γmm (frag_item pb)) with
    | inleft (τ↾Hτ) => cast_if_and (decide (frozen (frag_item pb)))
       (decide (frag_index pb < bit_size_of (Γmm.1) (τ.*)))
    | inright Hτ => right _
    end; abstract (destruct Γmm; first
  ...
  Global Instance ptr_bit_refine: Refine Ti (ptr_bit Ti) := λ Γ f pb1 pb2, ∃ τ,
    frag_item pb1 ⊑{Γ,f} frag_item pb2 : τ ∧
...
Lemma ptr_bit_valid_weakly_valid Γ m pb : ✓{Γ,Some m} pb → ✓{Γ,None} pb.
Proof.
  unfold valid, ptr_bit_valid. naive_solver eauto using ptr_typed_weakly_typed.
Qed.
Lemma ptr_bit_valid_weaken Γ1 Γ2 m1 m2 pb :
  ✓ Γ1 → ✓{Γ1,Some m1} pb → Γ1 ⊆ Γ2 →
...
Lemma ptr_to_bits_length Γ mm p τ :
  (Γ,mm) ⊢ p : τ → length (ptr_to_bits Γ p) = bit_size_of Γ (τ.* ).
Proof. intros. erewrite ptr_to_bits_length_alt, type_of_correct; eauto. Qed.
Lemma ptr_to_bits_valid Γ mm p τ : (Γ,mm) ⊢ p : τ → ✓{Γ,mm}* (ptr_to_bits Γ p).
Proof.
...
Qed.
Lemma ptr_of_bits_Exists_Forall_typed Γ mm τ pbs :
  is_Some (ptr_of_bits Γ τ pbs) → Exists ✓{Γ,mm} pbs → ✓{Γ,mm}* pbs.
Proof.
...
Qed.
Lemma ptr_of_bits_typed_frozen Γ mm p τ pbs :
  ptr_of_bits Γ τ pbs = Some p → ✓{Γ,mm}* pbs → (Γ,mm) ⊢ p : τ ∧ frozen p.
Proof.
  unfold ptr_of_bits; intros; simplify_option_equality.
  destruct (Forall_of_fragments (bit_size_of Γ (type_of p.* ))
    (✓{Γ,mm}) p pbs 0) as (?&?&?&?); eauto using type_of_typed.
  by apply Nat.neq_0_lt_0, bit_size_of_base_ne_0.
Qed.
Lemma ptr_of_bits_typed Γ mm p τ pbs :
  ptr_of_bits Γ τ pbs = Some p → ✓{Γ,mm}* pbs → (Γ,mm) ⊢ p : τ.
Proof. intros. eapply ptr_of_bits_typed_frozen; eauto. Qed.
Lemma ptr_of_bits_frozen Γ mm p τ pbs :
  ptr_of_bits Γ τ pbs = Some p → ✓{Γ,mm}* pbs → frozen p.
Proof. intros. eapply ptr_of_bits_typed_frozen; eauto. Qed.
...
Lemma ptr_to_of_bits Γ mm p τ pbs :
  ptr_of_bits Γ τ pbs = Some p → ✓{Γ,mm}* pbs → ptr_to_bits Γ p = pbs.
Proof.
...
Qed.
Lemma ptr_of_to_bits_typed Γ mm p τ :
  (Γ,mm) ⊢ p : τ → ptr_of_bits Γ τ (ptr_to_bits Γ p) = Some (freeze true p).
...
Qed.
Lemma ptr_to_bits_weaken Γ1 Γ2 mm p τ :
  ✓ Γ1 → (Γ1,mm) ⊢ p : τ → Γ1 ⊆ Γ2 → ptr_to_bits Γ1 p = ptr_to_bits Γ2 p.
Proof.
...
(** ** Refinements *)
Lemma ptr_bit_refine_id Γ mm pb : ✓{Γ,mm} pb → pb ⊑{Γ} pb.
Proof. intros (σ&?&?&?); exists σ; eauto using ptr_refine_id. Qed.
Lemma ptr_bit_refine_compose Γ f g pb1 pb2 pb3 :
  pb1 ⊑{Γ,f} pb2 → pb2 ⊑{Γ,g} pb3 → pb1 ⊑{Γ,f ◎ g} pb3.
Proof.
  intros (τ1&?&?&?&?) (τ2&?&?&?&?); exists τ1.
  assert (τ1 = τ2) by (by erewrite <-(ptr_refine_type_of_r _ _ _ _ τ1),
    <-(ptr_refine_type_of_l _ _ _ _ τ2) by eauto); subst.
  eauto using ptr_refine_compose with congruence.
Qed.
Lemma ptr_bit_refine_valid_l Γ f pb1 pb2 : ✓ Γ → pb1 ⊑{Γ,f} pb2 → ✓{Γ,None} pb1.
Proof. intros ? (τ&?&?&?&?). exists τ; eauto using ptr_refine_typed_l. Qed.
Lemma ptr_bit_refine_valid_r Γ f pb1 pb2 : ✓ Γ → pb1 ⊑{Γ,f} pb2 → ✓{Γ,None} pb2.
Proof.
  intros ? (τ&?&?&?&?); exists τ; erewrite <-ptr_refine_frozen by eauto;
    eauto using ptr_refine_typed_r with congruence.
Qed.
Lemma ptr_bits_refine_valid_l Γ f pbs1 pbs2 :
  ✓ Γ → pbs1 ⊑{Γ,f}* pbs2 → ✓{Γ,None}* pbs1.
Proof. induction 2; eauto using ptr_bit_refine_valid_l. Qed.
Lemma ptr_bits_refine_valid_r Γ f pbs1 pbs2 :
  ✓ Γ → pbs1 ⊑{Γ,f}* pbs2 → ✓{Γ,None}* pbs2.
Proof. induction 2; eauto using ptr_bit_refine_valid_r. Qed.
Lemma ptr_bit_refine_unique Γ f pb1 pb2 pb3 :
  pb1 ⊑{Γ,f} pb2 → pb1 ⊑{Γ,f} pb3 → pb2 = pb3.
Proof.
  destruct pb2, pb3; intros (τ1&?&?&?&?) (τ2&?&?&?&?);
    f_equal'; eauto using ptr_refine_unique with congruence.
Qed.
Lemma ptr_bits_refine_unique Γ f pbs1 pbs2 pbs3 :
  pbs1 ⊑{Γ,f}* pbs2 → pbs1 ⊑{Γ,f}* pbs3 → pbs2 = pbs3.
Proof.
  intros Hpbs. revert pbs3. induction Hpbs; inversion_clear 1;
    f_equal; eauto using ptr_bit_refine_unique.
Qed.
Lemma ptr_bit_refine_eq Γ pb1 pb2 : pb1 ⊑{Γ} pb2 → pb1 = pb2.
Proof.
  destruct pb1, pb2; intros (?&?&?&?&?); simplify_equality';
    f_equal; eauto using ptr_refine_eq.
Qed.
Lemma ptr_bits_refine_eq Γ pbs1 pbs2 : pbs1 ⊑{Γ}* pbs2 → pbs1 = pbs2.
Proof. induction 1; f_equal; eauto using ptr_bit_refine_eq. Qed.
Lemma ptr_to_bits_refine Γ f p1 p2 σ :
  p1 ⊑{Γ,f} p2 : σ → ptr_to_bits Γ p1 ⊑{Γ,f}* ptr_to_bits Γ p2.
Proof.
...
Qed.
Lemma ptr_of_bits_refine Γ f σ pbs1 pbs2 p1 :
  ptr_of_bits Γ σ pbs1 = Some p1 → pbs1 ⊑{Γ,f}* pbs2 →
  ∃ p2, ptr_of_bits Γ σ pbs2 = Some p2 ∧ p1 ⊑{Γ,f} p2 : σ.
Proof.
  revert pbs1 pbs2 p1. assert (∀ p1 p2 pbs1 pbs2,
    p1 ⊑{Γ,f} p2 : σ → frozen p1 → pbs1 ⊑{Γ,f}* pbs2 →
    fragmented (bit_size_of Γ (σ.*)) p1 1 pbs1 →
    fragmented (bit_size_of Γ (σ.*)) p2 1 pbs2).
  { intros p1 p2 pbs1 pbs2 ??? ->. apply (ptr_bits_refine_unique Γ f
