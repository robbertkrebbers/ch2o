...
Inductive freeable A := Available : A → freeable A | Freed.
Arguments Available {_} _.
Arguments Freed {_}.

Instance freeable_separation_ops `{SeparationOps A} :
    SeparationOps (freeable A) := {
  sep_valid x := match x with Available x => sep_valid x | Freed => True end;
  sep_subseteq x y :=
    match x, y with
    | Available x, Available y => x ⊆ y | Freed, Freed => True
    | Available x, Freed => x = ∅ | Freed, Available _ => False
    end;
  sep_empty := Available ∅;
  sep_disjoint x y :=
    match x, y with
    | Available x, Available y => x ⊥ y | Available x, Freed => x = ∅
    | Freed, Available y => y = ∅ | Freed, Freed => False end;
  sep_union x y :=
    match x, y with
    | Available x, Available y => Available (x ∪ y) | _, _ => Freed
    end;
  sep_difference x y :=
    match x, y with
    | Available x, Available y => Available (x ∖ y)
    | Freed, Freed => Available ∅ | _, _ => x
    end;
  sep_splittable x :=
    match x with Available x => sep_splittable x | _ => False end;
  sep_half x := match x with Available x => Available (½ x) | _ => Freed end;
  sep_unmapped x := match x with Available x => sep_unmapped x | _ => True end;
  sep_unshared x := match x with Available x => sep_unshared x | _ => True end
}.
Proof.
  * intros [?|]; apply _.
  * intros [?|] [?|]; apply _.
  ...
  * intros [?|]; apply _.
Defined.
Instance freeable_separation {A : Set} `{Separation A} : Separation (freeable A).
Proof.
  split.
  * by eexists Freed.
  * sep_unfold; intros [][]; naive_solver
      eauto using sep_disjoint_valid_l, sep_empty_valid.
  * sep_unfold; intros [][]; eauto using sep_union_valid.
  * sep_unfold; intros []; eauto using sep_disjoint_empty_l.
  * sep_unfold; intros [] ?; f_equal; auto using sep_left_id.
  * sep_unfold; intros [][]; naive_solver.
  * sep_unfold; intros [][] ?; f_equal; auto using sep_commutative'.
  * sep_unfold; intros [][][] ??; naive_solver eauto using sep_disjoint_ll,
      sep_positive_l', sep_disjoint_empty_l, sep_empty_valid.
  * sep_unfold; intros [][][] ??; naive_solver eauto using
      sep_disjoint_move_l, sep_positive_l', sep_left_id, sep_empty_valid.
  * sep_unfold;  intros [][][] ??; f_equal; auto using sep_associative'.
  * sep_unfold;  intros [][] ??;
      f_equal; naive_solver eauto using sep_positive_l'.
  * sep_unfold; intros [][][] ???;
      f_equal; naive_solver eauto using sep_cancel_l'.
  * sep_unfold; intros [][] ?; eauto using sep_union_subseteq_l'.
  * sep_unfold; intros [][]; naive_solver eauto using
      sep_disjoint_difference', sep_disjoint_empty_l, sep_empty_valid.
  * sep_unfold; intros [][] ?;
     f_equal; naive_solver eauto using sep_union_difference.
  * sep_unfold; intros []; naive_solver eauto using sep_splittable_union'.
  * sep_unfold; intros [] []; naive_solver eauto using sep_splittable_weaken.
  * sep_unfold; intros [] ?;
      f_equal; naive_solver eauto using sep_disjoint_half'.
  * sep_unfold; intros [] ?; f_equal; naive_solver eauto using sep_union_half.
  * sep_unfold; intros [][] ??;
      f_equal; naive_solver eauto using sep_union_half_distr'.
  * sep_unfold; intros [] ?; auto using sep_unmapped_valid.
  * sep_unfold; by apply sep_unmapped_empty.
  * sep_unfold; intros [][]; naive_solver
      eauto using sep_unmapped_weaken, sep_unmapped_empty.
  * sep_unfold; intros [][] ??; eauto using sep_unmapped_union_2'.
  * sep_unfold; intros [x|].
    { rewrite sep_unshared_spec'; split.
      + intros [??]; split; auto. intros [y|]; auto.
      + intros [? Hx]; split; auto. intros y ?. by apply (Hx (Available y)). }
    split; [|done]. split; auto.
    intros [?|] ?; auto using sep_unmapped_empty_alt.
Qed.
